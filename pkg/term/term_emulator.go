...
func isAnsiCommandChar(b byte) bool {
	if ANSI_COMMAND_FIRST <= b && b <= ANSI_COMMAND_LAST && b != ANSI_ESCAPE_SECONDARY {
		return true
	}
...
func (tw *terminalWriter) Write(p []byte) (n int, err error) {
	...
	if !tw.inSequence {
		...
		assert(0 == len(tw.command), "command can't be non empty : command=%+v \nrest=%+v", tw.command, p[start:])
		if len(p[start:]) > 0 {
...
func parseAnsiCommand(command []byte) *ansiCommand {
	assert(command[0] == ANSI_ESCAPE_PRIMARY, "Must start with escape %+v", command)
	assert(len(command) > 1, "Must have at least two char", command)
